...
> ⚠️ **落地策略说明**：本文描述的目标架构中，自研的 `nove-ai` 独立服务目前处于**中后期规划**阶段（仓库尚未实现）。**在独立投入自研前，我们将优先采用 OpenClaw、Hermes 等成熟 Agent 软件作为外部智能计算引擎，由其代办关联发现、图谱构建等复杂的预处理任务**。当前极少量轻量级 AI 调用则暂由 `nove-api` 内嵌的 `LlmService` 承担。组件与能力实际状态见 [能力状态总览](/guide/capabilities)。
...
因此，nove 将 AI 计算层设计为 **独立服务（远期为自研的 nove-ai，早期则直接使用 OpenClaw / Hermes 等外部 Agent 系统代办）**。
...
| 重量异步 | Embedding、语义关联、知识图谱、批量总结、用户画像 | 早期借用 OpenClaw/Hermes 编排，远期归属 nove-ai（Python），Job 化 |

### 3.3 nove-ai 的职责边界

智能计算层（外部 Agent 软件 或 远期 `nove-ai`）**只负责计算，不负责业务真相**：
- 数据预处理与关联发现
- 知识图谱 / 语义层构建
- 批量转写总结与画像
- 受控文本生成
